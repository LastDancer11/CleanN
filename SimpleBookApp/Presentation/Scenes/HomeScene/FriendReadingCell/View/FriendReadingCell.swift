...
class FriendReadingCell: UITableViewCell {
    ...
    private func configureDataSource() {
        ...
        dataSource = CellDataSource(with: collectionView, books: books, viewModel: viewModel, cellToDeque: FriendReadingItemCell.self)
    }
    
    private func setUpCollectionView() {
        let layout = UICollectionViewFlowLayout()
        layout.scrollDirection = .horizontal
        layout.itemSize = CGSize(width: 320, height: 260)
...
extension FriendReadingCell: UICollectionViewDelegateFlowLayout {
