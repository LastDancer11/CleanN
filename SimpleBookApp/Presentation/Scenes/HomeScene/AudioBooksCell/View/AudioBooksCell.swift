...
class AudioBooksCell: UITableViewCell {
    ...
    private func setupLayout() {
        collectionView.registerNib(class: AudioBooksItemCell.self)
    }
    
    private func configureDataSource() {
        viewModel = CellViewModel(coordinator: coordinator!)
        dataSource = CellDataSource(with: collectionView, books: books, viewModel: viewModel, cellToDeque: AudioBooksItemCell.self)
    }
...
extension AudioBooksCell: UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        return CGSize(width: 160, height: 274)
    }
}
